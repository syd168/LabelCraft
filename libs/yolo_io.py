...
class YOLOWriter:
    ...
    def save(self, class_list=[], target_file=None):
        ...
        if target_file is None:
            ...
            # Save classes.txt in parent directory (project root) instead of annotations directory
            anno_dir = os.path.dirname(os.path.abspath(self.filename))
            project_root = os.path.dirname(anno_dir)
            ...
            # Save classes.txt in parent directory (project root) instead of annotations directory
            anno_dir = os.path.dirname(os.path.abspath(target_file))
            project_root = os.path.dirname(anno_dir)
            classes_file = os.path.join(project_root, "classes.txt")
            out_class_file = open(classes_file, 'w')
...
class YoloReader:
    ...
    def __init__(self, file_path, image, class_list_path=None):
        ...
        if class_list_path is None:
            # Look for classes.txt in project root (parent of annotations directory)
            anno_dir = os.path.dirname(os.path.realpath(self.file_path))
            project_root = os.path.dirname(anno_dir)
            self.class_list_path = os.path.join(project_root, "classes.txt")
            
            # Fallback: if not found in project root, try annotations directory
            if not os.path.exists(self.class_list_path):
                self.class_list_path = os.path.join(anno_dir, "classes.txt")
        else:
    ...
    def parse_yolo_format(self):
        ...
        for bndBox in bnd_box_file:
            ...
            # Compatible with YOLO-Pose lines (extra keypoint fields ignored here).
            if len(parts) < 5:
                continue
