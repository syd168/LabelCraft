...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # Ensure it's a valid LabelFileFormat enum
        if not isinstance(self.label_file_format, LabelFileFormat):
            print(f"Warning: Invalid format from settings ({self.label_file_format}), resetting to PASCAL_VOC")
        ...
        # Clean up any prompt text that might have been accidentally added to label history
        self.label_hist = [label for label in self.label_hist if label not in ['Enter object label', 'enter object label']]
        ...
        self.file_list_widget.setSelectionMode(QListWidget.SelectionMode.ExtendedSelection)  # Enable multi-selection
        ...
        verify = action(self.get_str('verifyImg'), self.verify_image,
                        'V', 'verify', self.get_str('verifyImgDetail'),
                        checkable=True)

        # Batch verify all images
        verify_all = action(self.get_str('verifyAll'), self.verify_all_images,
                           None, 'verify', self.get_str('verifyAllDetail'))
        
        # Set selected as verified/unverified
        set_verified = action(self.get_str('setVerified'), lambda: self.set_selected_verified(True),
        ...
        def get_format_meta(format):
            ...
            elif format == LabelFileFormat.LABELCRAFT_JSON:
                return '&LabelCraft JSON', 'format_createml'  # Use createml icon as placeholder
        ...
        # Save toolbar toggle action for language switching
    ...
    def get_current_image_list(self):
        """
        Get the current image list based on mode (project or directory).
        
        Returns:
            list: List of image paths
        """
        if self.current_project:
            # Project mode: get from pending list widget
            image_list = []
            for i in range(self.pending_list_widget.count()):
                image_list.append(self.pending_list_widget.item(i).text())
            return image_list
        else:
            # Directory mode: use m_img_list
            return self.m_img_list
    ...
    def change_language(self, locale):
        ...
        if hasattr(self.actions, 'filterUnverified'):
            ...
            self.actions.filterUnverified.setToolTip(self.get_str('filterUnverifiedDetail'))
        
        # Update toggle labels action - always use showAllBox text (it's a toggle)
        if hasattr(self.actions, 'toggleLabels'):
        ...
        # Update help actions by reference
        if hasattr(self.actions, 'tutorialDefault'):
            self.actions.tutorialDefault.setText(self.get_str('tutorialDefault'))
            self.actions.tutorialDefault.setToolTip(self.get_str('tutorialDetail'))
        ...
        if hasattr(self, 'show_shortcut'):
            ...
            self.show_shortcut.setToolTip(self.get_str('shortcut'))

        # Update dock toggle action (show/hide labels panel)
        if hasattr(self.actions, 'labels'):
            self.actions.labels.setText(self.get_str('showHide'))
        
        # Update toolbar toggle action (show/hide toolbar)
    ...
    def update_completed_annotations_list(self):
        ...
        ext_map = {
            ...
            LabelFileFormat.LABELCRAFT_JSON: '.json',
            LabelFileFormat.YOLO: '.txt',
            LabelFileFormat.CREATE_ML: '.json'
        }
        ext = ext_map.get(self.label_file_format, '.xml')
        
        # Check if we should show verification status
        show_status = hasattr(self.actions, 'filterUnverified') and self.actions.filterUnverified.isChecked()
        
        try:
            # Get all annotation files and sort them for consistent ordering
            annotation_files = []
            ...
            for filename in annotation_files:
                ...
                # If showing verification status, check if file is verified
                if show_status:
                    filepath = os.path.join(anno_dir, filename)
                    is_verified = self._check_if_verified(filepath)
                    if is_verified:
                        # Set green color for verified files
                        item.setForeground(QColor(0, 128, 0))  # Green
                        item.setToolTip('Verified')
                    else:
                        item.setToolTip('Not verified')
    ...
    def _check_if_verified(self, annotation_file):
        ...
        try:
            ...
            if ext == '.xml':
                from libs.pascal_voc_io import PascalVocReader
                reader = PascalVocReader(annotation_file)
                return reader.verified
            elif ext == '.json':
                import json
                with open(annotation_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                return data.get('metadata', {}).get('verified', False)
        except:
            pass
        return False
    ...
    def save_labels(self, annotation_file_path):
        """
        Save annotations in configured format (VOC XML or LabelCraft JSON).
        
        This ensures:
        - Verified status is preserved
        - All metadata is stored
        - Easy to convert to other formats later
        
        Args:
            annotation_file_path: Base path without extension
        ...
        if self.label_file_format == LabelFileFormat.LABELCRAFT_JSON:
            ...
            # Default to VOC XML for backward compatibility
        ...
        try:
            ...
            if self.label_file_format == LabelFileFormat.LABELCRAFT_JSON:
                self.label_file.save_json_format(
    ...
    def new_shape(self):
        ...
        if text is not None:
            ...
            if text and text not in ['Enter object label', 'enter object label']:
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            # Update verify action checkbox state based on loaded file
            if hasattr(self.actions, 'verify'):
                self.actions.verify.setChecked(self.canvas.verified)
    ...
    def _migrate_annotations(self, old_dir, new_dir=None):
        ...
        current_format_name = {
            ...
            LabelFileFormat.LABELCRAFT_JSON: 'LABELCRAFT_JSON',
    ...
    def update_progress_info(self):
        ...
        annotated = sum(1 for img_path in image_list if self._has_annotation(img_path))
    ...
    def jump_to_next_unannotated(self):
        ...
        image_list = self.get_current_image_list()
        if not image_list or not self.file_path:
            return
        ...
        for i in range(current_idx + 1, total):
            if not self._has_annotation(image_list[i]):
                self.cur_img_idx = i
        ...
        for i in range(0, current_idx):
            if not self._has_annotation(image_list[i]):
                ...
                QMessageBox.information(
                    self,
                    self.get_str('tip'),
                    self.get_str('jumpedToFirstUnannotated').format(total - sum(1 for img in image_list if self._has_annotation(img)))
                )
                return
    ...
    def toggle_show_verification_status(self):
        """Toggle showing verification status color in completed annotations list"""
        # Refresh the completed annotations list to apply/remove colors
        self.update_completed_annotations_list()
    
    def verify_image(self, _value=False):
        """Toggle verification status for current image (does not auto-save)"""
        if self.file_path is not None:
            try:
                self.label_file.toggle_verify()
            except AttributeError:
                # If the labelling file does not exist yet, create it
                self.save_file()
            ...
            # Update verify action checkbox state
            if hasattr(self.actions, 'verify'):
                self.actions.verify.setChecked(self.canvas.verified)
            
            # Update status bar with verification state
            if self.canvas.verified:
                self.statusBar().showMessage(self.get_str('imageVerified'), 3000)
            else:
                self.statusBar().showMessage(self.get_str('imageUnverified'), 3000)
    ...
    def verify_all_images(self):
        ...
        # Get all annotation files from the list
        annotation_files = []
        for i in range(self.file_list_widget.count()):
            filename = self.file_list_widget.item(i).text()
            filepath = os.path.join(anno_dir, filename)
        ...
        for filename, filepath in annotation_files:
            try:
                ...
                if ext == '.xml':
                    ...
                    # Update verified attribute (always set to 'yes' for verify_all)
        ...
        # Refresh current image if loaded
        if self.file_path:
    ...
    def toggle_filter_unverified(self):
        ...
        is_filtered = self.actions.filterUnverified.isChecked()
        
        if is_filtered:
            # Get image list from current context
            if self.current_project:
                original_list = []
                for i in range(self.pending_list_widget.count()):
                ...
                original_list = self.m_img_list
            else:
                QMessageBox.warning(self, self.get_str('warningTitle'), self.get_str('noImagesInDir'))
                self.actions.filterUnverified.setChecked(False)
                return
            ...
            # Filter to show only unverified images (based on annotation files)
            unverified_list = []
            for img_path in original_list:
                ...
                # Only include if NOT verified (includes images without annotation)
                if not is_verified:
                    unverified_list.append(img_path)
            
            # Store original list and apply filter
            if not hasattr(self, '_original_img_list'):
                self._original_img_list = original_list.copy()
            
            # Update the appropriate list based on mode
            if self.current_project:
                # For project mode, update pending list widget
                self.pending_list_widget.clear()
                for img_path in unverified_list:
                    self.pending_list_widget.addItem(img_path)
            else:
                # For directory mode, update m_img_list
                self.m_img_list = unverified_list
                self.img_count = len(unverified_list)
            
            # Show message
            total = len(original_list)
            self.statusBar().showMessage(
                self.get_str('showingUnverified').format(len(unverified_list), total),
                5000
            )
            
            # Reload current image if it's still in the filtered list
            if self.file_path and self.file_path in unverified_list:
                self.load_file(self.file_path)
            elif unverified_list:
                # Load first unverified image
                if self.current_project:
                    self.cur_img_idx = 0
                else:
                    self.cur_img_idx = 0
                self.load_file(unverified_list[0])
        else:
            # Restore original list
            if hasattr(self, '_original_img_list'):
                original_list = self._original_img_list.copy()
                
                if self.current_project:
                    # For project mode, restore pending list widget
                    self.pending_list_widget.clear()
                    for img_path in original_list:
                        self.pending_list_widget.addItem(img_path)
                else:
                    # For directory mode, restore m_img_list
                    self.m_img_list = original_list.copy()
                    self.img_count = len(self.m_img_list)
                
                delattr(self, '_original_img_list')
                
                self.statusBar().showMessage(
                    self.get_str('filterCleared').format(len(original_list)),
                    3000
                )
    ...
    def open_next_image(self, _value=False):
        ...
        if self.file_path is None:
            ...
            if self.cur_img_idx + 1 < self.img_count and self.cur_img_idx + 1 < len(image_list):
                self.cur_img_idx += 1
                filename = image_list[self.cur_img_idx]
    ...
    def edit_project_dialog(self):
        ...
        # Set format (using radio buttons) - only XML and JSON
        format_map = {
            'PASCAL_VOC': dialog.format_voc,
            'LABELCRAFT_JSON': dialog.format_json
        }
    ...
    def _migrate_and_convert_annotations(self, old_dir, new_dir, old_format, new_format, mode='copy'):
        ...
        Args:
            ...
            old_format: Source format (PASCAL_VOC, LABELCRAFT_JSON, YOLO, CREATE_ML)
            ...
            for filename in os.listdir(old_dir):
                if filename.lower().endswith(old_ext):
                    ...
                    if old_format != new_format:
                        # Convert using AnnotationConverter
                        ...
                        try:
                            ...
                            # Map format names to converter strings
                            format_str_map = {
                                'PASCAL_VOC': 'voc',
                                'LABELCRAFT_JSON': 'json',
                                'YOLO': 'yolo',
                            ...
                            input_format_str = format_str_map.get(old_format, 'voc')
                            output_format_str = format_str_map.get(new_format, 'voc')
                            
                            # Get classes list for YOLO/COCO formats
                            ...
                            # Remove source file if in move mode
                            if mode == 'move':
                                os.remove(src_path)
    ...
    def load_project(self, project):
        ...
        format_map = {
            ...
            'LABELCRAFT_JSON': LabelFileFormat.LABELCRAFT_JSON
        }
        format_names = {
    ...
    def export_annotations_dialog(self, _value=False):
        ...
        def detect_format(file_path):
            ...
            if ext == '.xml':
                ...
                # Try to detect if it's LabelCraft JSON, CreateML or COCO
                try:
                    with open(file_path, 'r') as f:
                        data = json.load(f)
                    # LabelCraft JSON has 'version' and 'image' fields
                    if 'version' in data and 'image' in data:
                        return 'json'  # LabelCraft JSON
                    elif isinstance(data, list) and len(data) > 0 and 'image' in data[0]:
                        return 'createml'
        ...
        try:
            ...
            for anno_file_path in annotation_files:
                ...
                dest_anno_file = base_name + output_ext
                ...
                try:
                    ...
                    print(f"Converting: {anno_filename} ({input_format_str}) -> {dest_anno_file} ({output_format_str})")
                    ...
                    try:
                        ...
                        if success and os.path.exists(dest_anno_path):
                            file_size = os.path.getsize(dest_anno_path)
                            print(f"  ✓ Created: {dest_anno_path} ({file_size} bytes)")
                            exported_count += 1
                        elif not success:
                            # File was skipped due to no valid annotations
                            print(f"  ⊘ Skipped: No valid labels in classes list")
                            skipped_count += 1
                        else:
...
def get_main_app(argv=None):
    ...
    Returns:
        tuple: (QApplication, MainWindow) instances
    """
    if not argv:
        argv = []
