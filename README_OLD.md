...
- 📁 Multiple annotation format support:
  - **PASCAL VOC** (XML format) - Traditional format for object detection
  - **YOLO** (TXT format) - For YOLO series models
  - **CreateML** (JSON format) - For Apple CreateML framework
  - **COCO** (JSON format) - Microsoft COCO dataset format
  - **CSV** (CSV format) - Universal spreadsheet format
- 🔄 Unified annotation converter - Convert between all 5 formats seamlessly
...
You can select different annotation formats in the right panel output settings:
- **PascalVOC**: Generates XML files, suitable for most object detection frameworks (Faster R-CNN, SSD, etc.)
- **YOLO**: Generates TXT files, suitable for YOLO series models (YOLOv5, YOLOv8, etc.)
- **CreateML**: Generates JSON files, suitable for Apple CreateML framework
- **COCO**: Generates JSON files in Microsoft COCO format
- **CSV**: Generates CSV files, universal format for data analysis

### Format Conversion

LabelCraft includes a powerful annotation converter that can convert between all supported formats:
...
# Convert from VOC to YOLO
converter.convert('path/to/annotations', 'voc', 'yolo', 'output/dir')

# Convert from YOLO to COCO
converter.convert('path/to/yolo', 'yolo', 'coco', 'output/dir')
```

Supported conversions:
- PASCAL VOC ↔ YOLO ↔ CreateML ↔ COCO ↔ CSV (all combinations)
...
├── libs/                # Core libraries
│   ├── canvas.py        # Canvas component for drawing
│   ├── shape.py         # Shape classes (Rectangle, etc.)
│   ├── labelFile.py     # Annotation file handling
│   ├── annotation_converter.py  # Unified format converter
│   ├── i18n_engine.py   # Internationalization engine
│   ├── pascal_voc_io.py # VOC format I/O
│   ├── yolo_io.py       # YOLO format I/O
│   ├── create_ml_io.py  # CreateML format I/O
│   ├── coco_io.py       # COCO format I/O
│   ├── csv_io.py        # CSV format I/O
│   └── json_io.py       # JSON format I/O
...
├── doc/                 # Documentation
│   ├── tutorial.md      # English user guide
│   ├── tutorial_zh-CN.md # Chinese user guide
│   └── TRANSLATION_GUIDE.md # Translation guide
├── build-tools/         # Build scripts for different platforms
└── .github/workflows/   # GitHub Actions CI/CD configuration
```

### Adding New Language

LabelCraft uses a modern JSON-based internationalization system:

1. Create a new language file in `locales/` directory (e.g., `es-ES.json` for Spanish)
2. Copy an existing JSON file as template (e.g., `en.json`)
3. Translate all string values while maintaining the nested key structure
4. The program will automatically detect and load the new language
5. No recompilation needed - translations are loaded at runtime
...
```json
{
  ...
  "edit": {
    "createRectBox": "Create RectBox"
  }
}
```

For detailed translation guidelines, see [TRANSLATION_GUIDE.md](doc/TRANSLATION_GUIDE.md)
...
A: LabelCraft includes a built-in annotation converter supporting all 5 formats (PASCAL VOC, YOLO, CreateML, COCO, CSV). You can use it programmatically:

```python
from libs.annotation_converter import AnnotationConverter

converter = AnnotationConverter()
# Convert entire directory from VOC to YOLO
converter.convert('input_dir', 'voc', 'yolo', 'output_dir')
```

Or use the command-line tool:
```bash
python -m libs.annotation_converter --input input_dir --input_format voc --output_format yolo --output output_dir
```
